...
def test_valid_long_input_is_handled(client):
    ...
    assert response.status_code == 200
    assert response.json()["safe"]  # Assuming long text is safe if it doesn't contain toxic content
...
def test_special_characters_input_is_handled(client):
    ...
    assert response.status_code == 200
    assert response.json()["safe"]  # Assuming special characters alone are safe
...
def test_multilingual_input_is_handled(client):
    ...
    assert response.json()[
        "safe"
    ]  # Assuming multilingual text is safe if it doesn't contain toxic content
...
def test_multilingual_toxic_input_is_flagged(client):
    ...
    assert not response.json()[
        "safe"
    ]  # Toxic content in another language should be flagged as unsafe
...
def test_html_input_is_handled(client):
    ...
    assert response.json()[
        "safe"
    ]  # Assuming HTML input is safe if it doesn't contain toxic content
...
def test_unicode_input_is_handled(client):
    ...
    assert response.json()[
        "safe"
    ]  # Assuming Unicode text is safe if it doesn't contain toxic content
...
def test_input_with_emojis_is_handled(client):
    ...
    assert response.json()[
        "safe"
    ]  # Assuming text with emojis is safe if it doesn't contain toxic content
...
def test_input_with_newlines_is_handled(client):
    ...
    assert response.json()[
        "safe"
    ]  # Assuming text with newlines is safe if it doesn't contain toxic content
...
def test_input_with_tabs_is_handled(client):
    ...
    assert response.json()[
        "safe"
    ]  # Assuming text with tabs is safe if it doesn't contain toxic content
...
def test_input_with_whitespace_is_handled(client):
    ...
    assert response.json()[
        "safe"
    ]  # Assuming text with leading/trailing whitespace is safe if it doesn't contain toxic content
...
def test_clearly_toxic_input_is_flagged(client):
    ...
    assert response.status_code == 200
    assert not response.json()["safe"]  # Clearly toxic input should be flagged as unsafe
...
def test_clearly_safe_input_is_not_flagged(client):
    ...
    assert response.status_code == 200
    assert response.json()["safe"]  # Clearly safe input should not be flagged as unsafe
...
def test_input_with_mixed_content_is_handled(client):
    ...
    assert response.status_code == 200
    assert not response.json()["safe"]  # Mixed content with toxic parts should be flagged as unsafe
...
def test_pii_present_toxic_absent_is_flagged(client):
    ...
    assert response.status_code == 200
    assert not response.json()["safe"]  # PII present should be flagged as unsafe
...
def test_pii_present_toxic_present_is_flagged(client):
    ...
    assert not response.json()[
        "safe"
    ]  # Both PII and toxic content present should be flagged as unsafe
    assert "EMAIL_ADDRESS" in response.json()["flags"]
...
def test_email_address_detection(client):
    ...
    assert latency >= 0  # Ensure latency is a non-negative value
    assert response.status_code == 200
    assert not response.json()["safe"]  # Email address present should be flagged as unsafe
    assert "EMAIL_ADDRESS" in response.json()["flags"]
...
def test_phone_number_detection(client):
    ...
    assert latency >= 0  # Ensure latency is a non-negative value
    assert response.status_code == 200
    assert not response.json()["safe"]  # Phone number present should be flagged as unsafe
...
def test_credit_card_detection(client):
    ...
    assert latency >= 0  # Ensure latency is a non-negative value
    assert response.status_code == 200
    assert not response.json()["safe"]  # Credit card number present should be flagged as unsafe
...
def test_us_ssn_detection(client):
    ...
    assert latency >= 0  # Ensure latency is a non-negative value
    assert response.status_code == 200
    assert not response.json()["safe"]  # US SSN present should be flagged as unsafe
...
def test_person_name_detection(client):
    ...
    assert not response.json()["safe"]  # Person name present should be flagged as unsafe
...
def test_location_detection(client):
    ...
    assert response.json()[
        "safe"
    ]  # Location present should not be flagged as unsafe unless it's considered sensitive PII
...
def test_date_detection(client):
    ...
    assert response.json()[
        "safe"
    ]  # Date present should not be flagged as unsafe unless it's considered sensitive PII
...
def test_two_email_addresses_detection(client):
    ...
    assert response.status_code == 200
    assert "EMAIL_ADDRESS" in response.json()["flags"]
    assert not response.json()["safe"]  # Two email addresses present should be flagged as unsafe
...
def test_score_exactly_on_threshold_is_flagged(client):
    with patch("app.services.verdict.llm_output_validation") as mock_predict:
        ...
        assert not response.json()[
            "safe"
        ]  # Input with score exactly on threshold should be flagged as unsafe


def test_model_timeout_handling(client, monkeypatch):
    with patch("app.services.toxicity.func_timeout") as mock_predict:
        monkeypatch.setattr(toxicity.llm_output_validation.retry, "wait", wait_fixed(0)) 
...
def test_slow_model_timeout_handling(client, monkeypatch):
    with patch("app.services.toxicity.func_timeout") as mock_predict:
        monkeypatch.setattr(toxicity.llm_output_validation.retry, "wait", wait_fixed(0)) 
        mock_predict.side_effect = [
            ...
            FunctionTimedOut,
            {"toxicity": 0.1},
        ]  # First two calls timeout, third call succeeds
